...
def main():
    ...
    # Extract the GitHub Repository URL from the issue body
    url_match = re.search(rf"### GitHub Repository URL\s*\n\s*(https://github\.com/[^/\s]+/[^/\s]+)", content)
    if not url_match:
        print("Error: Could not extract a valid GitHub URL from submission.")
        sys.exit(1)
    
    git_url = url_match.group(1).strip().rstrip("/")
    
    # Parse username and repo name out of the URL
    ...
    try:
        ...
        #can through discovered JSON files until we find one with a name/id
        manifest_data = None
        chosen_file = None
        ...
        for file_name in json_files:
            ...
            try:
                ...
                # Check for your mandatory fallback identity criteria
        ...
        if not manifest_data:
            raise Exception("Found JSON files, but none of them contained 'id', 'name', or 'display_name' fields.")
        ...
        # Step 5: Merge into your mod.json database file
        json_file = "mod.json"
        ...
        print(f"Scraper Pipeline Execution Error: {e}")
        sys.exit(1)
